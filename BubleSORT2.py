...
yükselik = [200, 50, 130, 90, 250, 61, 110,
			88, 33, 80, 70, 159, 180, 20]

for i in range(len(yükselik)-1):
        for j in range(len(yükselik)-i -1):
            if yükselik[j] > yükselik[j+1]:
                t=yükselik[j]
                yükselik[j]=yükselik[j+1]
                yükselik[j+1]=yükselik[j]
